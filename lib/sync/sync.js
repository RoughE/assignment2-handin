...
function captureDirectoryState(directoryInfo,cb){
    ...
    var fileList = directoryInfo.fileList.filter(function(file) {
        return file !== '.dignore';
    });
    var path = directoryInfo.path;
    var dignorePath = './.dignore';
    if (fs.existsSync(dignorePath)) {
        var contents = fs.readFileSync(dignorePath, 'utf8');
        var patterns = contents.split('\n').filter(function(c) {
            return c !== '';  // filter out eof
        });
        if (patterns.length > 0) {
            if (patterns[0].trim() === '%regex'.trim()) {
                fileList = fileList.filter(function(file) {
                    for (var j = 0; j < patterns.length; j++) {
                        if (file.search(new RegExp(patterns[j])) > -1) {
                            return false;
                        }
                    }
                    return file !== '.dignore';
                ...
                fileList = glob.sync('*', {cwd: process.cwd() + '/' + path, ignore: patterns});
            }
        }
