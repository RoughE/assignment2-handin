...
// Tried to add functionality to check if file contents were equal. Currently not working.
 ...
 function filesMatchExactly(fstate1,fstate2){

 var filesMatch = filesMatchNameAndSize(fstate1,fstate2) && filesMatchContents(fstate1,fstate2);

 return filesMatch;
 }

 // Check if the contents of both files match
 function filesMatchContents(fstate1,fstate2){

     var f1buffer = "";
     var f2buffer = "";

     fs.readFile(fstate1, function(err, data) { // How do I get file path from fstate1?
         if (err) throw err;
         f1buffer = data;
     });

     fs.readFile(fstate2, function(err, data) { // How do I get file path from fstate2?
         if (err) throw err;
         f2buffer = data;
     });

     return f1buffer.toString() === f2buffer.toString();
 }
...
function compareDirectoriesOneWay(state1,state2,matcher){
    ...
    for(var name in state1){
        ...
        if(!onIgnoreList(name) && !matcher(fstate1,fstate2)) {
