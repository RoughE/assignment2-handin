...
function getDirectoryInfo(path, cb) {
    ...
    handler.list(path, function (files) {
        //Remove ignored files such that program is unaware of files it's told to ignore
...
function removeIgnoredFiles(files, path) {
    ...
    if (_.contains(files, dropboxIgnore)) {
        ...
        if (!ignoreList.lastModifiedDate || modified>ignoreList.lastModifiedDate){
            data = fs.readFileSync(uris.getPath(path) + "/" + dropboxIgnore, {'encoding': 'utf8'});
            ignoreList.files = data.split('\n');
            ignoreList.lastModifiedDate=modified;
        }
        _.each(ignoreList.files, function (ignoredFile) {
                if (files.indexOf(ignoredFile) > -1) {
                    files.splice(i, 1);
                }
        })
    }
