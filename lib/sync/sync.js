...
function getDirectoryInfo(path, cb){
    ...
    handler.list(path, function(files){
        ...
        remainingFiles=removeIgnoredFiles(files,handler,path);
        console.log("I'm out")
        cb({
...
function removeIgnoredFiles(files,handler,path){
    if(_.contains(files,dropboxIgnore)) {
        handler.readFile(path + "/" + dropboxIgnore, function (data) {
            text = base64util.decodeFromBase64(data);
            var filesToIgnore = text.split('\n');

            _.each(filesToIgnore, function (ignoredFile) {
