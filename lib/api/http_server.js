...
/* TODO: Consider rewriting to use native filesystem commands instead of
 * going through dnode server.
 */

/* TODO: Use https instead of http. This would require using a self-signed
 * key, but maybe that's better than nothing? ... Definitely shouldn't put a
 * private key in the public github repo, though.
 */
...
var progname = 'dropbox-http-server';

function parse_args() {
  var argv = require('yargs');

  argv.usage('usage: ' + progname + ' [options]');

  argv.alias('a', 'remote-address');
  argv.nargs('a', 1);
  argv.default('a', '127.0.0.1', '127.0.0.1 (localhost)');
  argv.describe('a', 'address of file server');

  argv.alias('p', 'remote-port');
  argv.nargs('p', 1);
  argv.default('p', '5004', '5004 (anticipated default port)');
  argv.describe('p', 'port on which to connect to file server');

  argv.alias('lp', 'listen-port');
  argv.nargs('lp', 1);
  argv.default('lp', 8080);
  argv.describe('lp', 'port on which to listen (for http requests)');

  argv.example(progname + ' -a 127.0.0.1 -p 5004 -lp 8080',
      'launch an http server listening on port 8080 funneling api requests ' +
      'through to a file server listening on 127.0.0.1:5004');
...
function main() {
  var argv = parse_args();
  var remote_addr = argv['remote-address'];
  if (remote_addr == 'localhost')
    remote_addr = '127.0.0.1';
  var remote_port = argv['remote-port'];
  var listen_port = argv['listen-port'];

  spawn_server(remote_addr, remote_port, listen_port);
}
